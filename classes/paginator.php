...
class Paginator{
	...
	public function __construct($total=0,$mid_range=7,$ipp_array=array(10,25,50,100,"All")) {
		...
		if($this->num_pages > 10) {
			$this->return = ($this->current_page > 1 And $this->total_items >= 10) ? "<li><a class=\"paginate\" href=\"$_SERVER[PHP_SELF]?page=".($this->current_page-1)."&ipp=$this->items_per_page$this->querystring\">Previous</a></li> ":"<li><span class=\"inactive\" href=\"#\">Previous</span></li> ";
			...
			for($i=1;$i<=$this->num_pages;$i++) {
				$this->return .= ($i == $this->current_page) ? "<li><a class=\"current\" href=\"#\">$i</a> ":"<li><a class=\"paginate\" href=\"$_SERVER[PHP_SELF]?page=$i&ipp=$this->items_per_page$this->querystring\">$i</a></li> ";
			}
	...
	public function display_items_per_page() {
		...
		return "<span class=\"paginate pagination_item\">Items per page:</span><select class=\"paginate pagination_item\"  onchange=\"window.location='$_SERVER[PHP_SELF]?page=1&amp;ipp='+this[this.selectedIndex].value+'$this->querystring';return false\">$items</select>\n";
	}
	public function display_jump_menu() {
		...
		return "<span class=\"paginate pagination_item\" >Page:</span><select class=\"paginate pagination_item\" onchange=\"window.location='$_SERVER[PHP_SELF]?page='+this[this.selectedIndex].value+'&amp;ipp=$this->items_per_page$this->querystring';return false\">$option</select>\n";
	}
